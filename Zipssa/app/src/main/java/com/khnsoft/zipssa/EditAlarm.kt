...
class EditAlarm : AppCompatActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		alarm_times.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
			...
			override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
				...
				if (!timesInitialized) {
					adapter = EditTimeRecyclerAdapter(this@EditAlarm, _jTimes)
				} else {
					adapter = EditTimeRecyclerAdapter(this@EditAlarm, DEFAULT_TIMES[position].asJsonArray)
				}
				val lm = LinearLayoutManager(this@EditAlarm)
		...
		edit_start_date.setOnClickListener {
			DatePickerDialog(this@EditAlarm, { view, year, month, dayOfMonth ->
		...
		edit_delete_btn.setOnClickListener {
			...
			val intent = Intent(this@EditAlarm, MyAlertPopup::class.java)
		...
		edit_confirm_btn.setOnClickListener {
			...
			if (radioGroup.getCheckedIndex() == -1) {
				Toast.makeText(this@EditAlarm, "증상을 선택해 주세요.", Toast.LENGTH_LONG).show()
			...
			if (lTimes.size > 0 && lRepeats.size == 0) {
				Toast.makeText(this@EditAlarm, "요일을 선택해 주세요", Toast.LENGTH_SHORT).show()
				return@setOnClickListener
			}
	...
	fun setupLabels() {
		...
		val myHandler = DBHandler.open(this@EditAlarm)
		...
		label@while (count < labelSize) {
			labelLine = AlarmLabelLine(this@EditAlarm)
			...
			for (i in 0..3) {
				...
				labelBox.id = 0
				labelItem = AlarmLabelItem(this@EditAlarm)
	...
	class EditTimeRecyclerAdapter(context: Context?, lTimes: JsonArray) :
