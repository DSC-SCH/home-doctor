	...
	inner class MainItemRecyclerAdapter(val lItems: JsonArray, val lAllItems: JsonArray, val lCounts: JsonArray) :
		...
		inner class ItemViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
			...
			fun setItemDetails(jItem: JsonObject) {
				...
				count.text = "${lTimes.size()} times a day"
			...
			fun setRemainDetails() {
				...
				if (nextIndex != -1) {
					...
					time_remain.text = "${if (nextTime > DateUtils.DAY_IN_MILLIS)
						"${nextTime / DateUtils.DAY_IN_MILLIS}일 ${nextTime % DateUtils.DAY_IN_MILLIS / (60 * 60 * 1000)} hours ${nextTime / (60 * 1000) % 60} mins"
					else
						"${nextTime % DateUtils.DAY_IN_MILLIS / (60 * 60 * 1000)} hours ${nextTime / (60 * 1000) % 60} mins"}"
