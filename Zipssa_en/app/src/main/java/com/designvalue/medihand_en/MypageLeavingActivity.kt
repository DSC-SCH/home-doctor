...
class MypageLeavingActivity : AppCompatActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		confirm_btn.setOnClickListener {
			...
			if (reason == -1) {
				Toast.makeText(this@MypageLeavingActivity, "Please select the reason for secession", Toast.LENGTH_LONG).show()
				return@setOnClickListener
			} else if (reason == 4 && reason_else.text.toString().isBlank()) {
				Toast.makeText(this@MypageLeavingActivity, "Please type the reason for secession", Toast.LENGTH_LONG).show()
				return@setOnClickListener
			}
			val data = MyAlertPopup.Data(AlertType.ALERT).apply {
				alertTitle = "Service Secession"
				alertContent = "Once deleted, accounts cannot be recovered."
				...
				confirmListener = View.OnClickListener {
					...
					when (UserData.accountType) {
						...
						AccountType.OFFLINE -> {
							...
							Toast.makeText(this@MypageLeavingActivity, "The internet connection is required for secession", Toast.LENGTH_LONG).show()
						}
					}
				}
			}
