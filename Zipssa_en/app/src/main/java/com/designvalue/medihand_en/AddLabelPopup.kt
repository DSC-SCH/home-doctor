	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		add_label_btn.setOnClickListener {
			if (label_title.text.isBlank()) {
				Toast.makeText(this@AddLabelPopup, "Please type the symptom.", Toast.LENGTH_LONG).show()
				return@setOnClickListener
			}

			val colorIndex = radioGroup.getCheckedIndex()
			if (colorIndex == -1) {
				Toast.makeText(this@AddLabelPopup, "Please choose a color.", Toast.LENGTH_LONG).show()
				return@setOnClickListener
			}
