	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		join_confirm_btn.setOnClickListener {
			...
			if (name_input.text.isBlank()) {
				isOK = false
				join_name_check_warning.text = "Please type your name"
			} else {
			...
			if (!gender_m.isChecked && !gender_f.isChecked) {
				isOK = false
				join_gender_check_warning.text = "Please select your gender"
			} else {
			...
			if (birthdayTime == BIRTHDAY_NO_SELECT) {
				isOK = false
				join_birthday_check_warning.text = "Please select your birthday"
			} else if (birthdayTime > System.currentTimeMillis()) {
				isOK = false
				join_birthday_check_warning.text = "Youth unser the age of 14 are restricted" +
						""
			} else {
			...
			if (!Checker.checkEmail(email_input.text.toString())) {
				isOK = false
				join_email_check_warning.text = "Please type a correct e-mail"
			} else {
			...
			if (validPh == null) {
				isOK = false
				join_ph_warning.text = "Please type a correct phone number"
			} else {
			...
			if (!checkContracts()) {
				isOK = false
				join_contract_warning.text = "You must accept all the terms and conditions to sign up."
			} else {
			...
			if (isOK) {
				...
				val data = MyAlertPopup.Data(AlertType.CONFIRM).apply {
					alertTitle = "Success"
					alertContent = "You are registered as a member of YakSon."
				}
