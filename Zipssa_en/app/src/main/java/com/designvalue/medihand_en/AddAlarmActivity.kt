	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		submit_btn.setOnClickListener {
			...
			if (radioGroup.getCheckedIndex() == -1) {
				Toast.makeText(this@AddAlarmActivity, "Please choose the symptom.", Toast.LENGTH_LONG).show()
			...
			if (lTimes.size > 0 && lRepeats.size == 0) {
				Toast.makeText(this@AddAlarmActivity, "Please choose days of the week.", Toast.LENGTH_SHORT).show()
				return@setOnClickListener
			}
			...
			if (UserData.careUser == null) {
				...
				data.alertTitle = "Success"
				data.alertContent = "The crew needs to re-run the application once to apply the changes."
