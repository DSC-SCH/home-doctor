...
import homedoctor.medicine.api.dto.medicine.MedicineSearchKeywordRequest;
...
public class MedicineApiController {
    ...
    public DefaultResponse searchMedicineByName(
            @RequestBody @Valid MedicineSearchKeywordRequest request) {
        try {

            List<Medicine> medicineList =
                    (List<Medicine>) medicineService.findByName(request.getKeyword()).getData();
            ...
            List<MedicineDto> medicineDtoList = medicineList.stream()
                    .map(m -> MedicineDto.builder()
                            ...
                            .name(m.getName())
                            .effect(m.getEffect())
                            .saveMethod(m.getSaveMethod())
                            .validDate(m.getValidDate())
                            .dosage(m.getDosage())
                            .precaution(m.getPrecaution())
                            .build())
    ...
    public DefaultResponse searchMedicineByKeyword(
        ...
        try {
            List<Medicine> medicineList =
                    (List<Medicine>) medicineService.findByKeyword(request.getKeyword()).getData();
